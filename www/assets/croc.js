// Generated by Haxe 4.0.0-preview.5+a37320c
(function () { "use strict";
...
Croco.prototype = {
	...
	,init: function() {
		...
		console.log("App.hx:114:","hello croquis last");
	}
	...
	,onMouseUp: function(e) {
		...
		console.log("App.hx:129:","up" + Std.string(this.croq.mz));
	}
	,setPointerEvent: function(e) {
		if(e.pointerType != "pen" && tablet_Tablet.pen() != null && tablet_Tablet.pen().pointerType != null) {
			e.pointerType = "pen";
			e.pressure = tablet_Tablet.pressure();
			if(tablet_Tablet.isEraser()) {
				console.log("App.hx:139:","eraser");
...
var croqmur__$CoolPoint_CoolPoint_$Impl_$ = {};
croqmur__$CoolPoint_CoolPoint_$Impl_$.__name__ = true;
croqmur__$CoolPoint_CoolPoint_$Impl_$._new = function(p) {
	return p;
};
croqmur__$CoolPoint_CoolPoint_$Impl_$.fromArray = function(a) {
	return croqmur__$CoolPoint_CoolPoint_$Impl_$._new({ x : a[0], y : a[1], press : a[2]});
};
var croqmur_Croq = function() {
	...
	this.trailong = 200;
	this.positions = [];
	this.memo = croqmur__$CoolPoint_CoolPoint_$Impl_$.fromArray([]);
	this.point = croqmur__$CoolPoint_CoolPoint_$Impl_$.fromArray([]);
};
croqmur_Croq.__name__ = true;
croqmur_Croq.prototype = {
	...
	,down: function(x,y,press) {
		this.store(croqmur__$CoolPoint_CoolPoint_$Impl_$.fromArray([x,y,-1]));
		if(this.tim != null) {
			this.tim.stop();
		}
	}
	...
	,move: function(x,y,press) {
		this.point = croqmur__$CoolPoint_CoolPoint_$Impl_$.fromArray([x,y,press]);
		this.store(this.point);
	}
	,drawCircle: function(ctx,_point,ratio,rgb) {
		if(rgb == null) {
			rgb = "204, 102, 153";
		}
		if(_point.press == -1) {
	...
	,drawTab: function(ctx,befPoint,curPoint,ratio,rgb) {
		if(rgb == null) {
			rgb = "204, 102, 153";
		}
		ctx.beginPath();
		...
		if(befPoint.press != -1) {
			...
			ctx.stroke();
			this.drawCircle(ctx,curPoint,ratio,"198, 88, 29");
		} else {
			this.drawCircle(ctx,this.point,ratio,"201, 185, 101");
		}
	}
	,render: function(ctx) {
		var ratio = 1.0;
		...
		while(_g < _g1) {
			...
			this.drawTab(ctx,this.positions[i - 1 > 0 ? i - 1 : i],this.positions[i],ratio,"43, 117, 153");
		}
		var _g2 = 0;
		var _g3 = this.mz.length;
		while(_g2 < _g3) {
			var i1 = _g2++;
			ratio = (i1 + 1) / this.positions.length;
			this.drawTab(ctx,this.mz[i1 - 1 > 0 ? i1 - 1 : i1],this.mz[i1],ratio,"86, 126, 62");
		}
	}
};
...
interact_KeyBoardManager.__name__ = true;
...
interact_KeyBoardManager.prototype = {
	...
	,addListener: function(key,func) {
		...
		this.listen();
		console.log("interact/KeyBoardManager.hx:52:",this.listeners.toString());
	}
	...
	,listen: function() {
		...
		this.restart();
		this.mok = function(e) {
			console.log("interact/KeyBoardManager.hx:64:","key code=" + e.keyCode);
			var code = e.keyCode;
...
var interact_NoteType = $hxEnums["interact.NoteType"] = { __ename__ : true, __constructs__ : ["Simple","Sticky"]
	,Simple: {_hx_index:0,__enum__:"interact.NoteType"}
	,Sticky: {_hx_index:1,__enum__:"interact.NoteType"}
};
var interact_Keynote = function() {
	interact_KeyBoardManager.getInstance().signal.handle($bind(this,this.onTime));
};
interact_Keynote.__name__ = true;
...
interact_Keynote.prototype = {
	onTime: function(s) {
		console.log("interact/KeyNote.hx:26:","onTime");
...
tablet_Tablet.__name__ = true;
tablet_Tablet.pen = function() {
	return new tablet_Tablet().plugin.penAPI;
};
tablet_Tablet.pressure = function() {
	var pen = tablet_Tablet.pen();
...
tablet_Tablet.isEraser = function() {
	var pen = tablet_Tablet.pen();
	if(pen != null) {
		return pen.isEraser;
	} else {
